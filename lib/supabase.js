...
const supabaseAnonKey = isPlaceholderSupabaseValue(rawSupabaseAnonKey)
  ? ''
  : rawSupabaseAnonKey;
const NATIVE_AUTH_REDIRECT_URL = 'bside://auth/callback';
const DEBUG_LOGS_ENABLED = typeof __DEV__ !== 'undefined' ? __DEV__ : false;
const LISTENING_EVENT_RETRY_DELAY_MS = 700;
const NOTIFICATION_ALLOWED_TYPES = new Set(['social']);
const NOTIFICATION_ALLOWED_ENTITY_TYPES = new Set([
  'profile',
  'review',
  'message',
...
const logSupabaseDebug = (scope, payload = {}) => {
  if (!DEBUG_LOGS_ENABLED) {
    return;
  }

  console.log('[supabase-sync]', scope, payload);
};

const logSupabaseError = (scope, error, payload = {}) => {
  console.error('[supabase-sync]', scope, {
...
const formatSupabaseErrorMessage = (
  ...
  if (
    ...
    return configuredUrl
      ? `No pudimos conectar con Supabase en ${configuredUrl}. Revisá si la URL del proyecto sigue siendo válida y si el proyecto está activo.`
      : 'No pudimos conectar con Supabase. Revisá la URL del proyecto y volvé a intentar.';
  }

  if (/non-existent domain|name could not be resolved|getaddrinfo/i.test(message)) {
    return configuredUrl
      ? `La URL de Supabase parece inválida o ya no existe: ${configuredUrl}.`
      : 'La URL de Supabase parece inválida o ya no existe.';
  }
...
export const SUPABASE_SETUP_STEPS = [
  ...
  'Crear buckets públicos o privados para avatars y wallpapers.',
  'Activar email auth y agregar http://localhost:19006 en Authentication > URL Configuration.',
  'Definir revisión manual para avatar y wallpaper antes de abrir perfiles públicos.',
];
...
const mapProfileRowToUser = (profileRow = {}, fallbackUser = {}) =>
  normalizeUser({
    ...
    birthDate: profileRow.birth_date || fallbackUser.birthDate || '',
    profileCompletedAt:
      profileRow.profile_completed_at || fallbackUser.profileCompletedAt || '',
    email: fallbackUser.email,
  });

const mapSessionUserToFallback = (sessionUser = {}, fallbackUser = {}) =>
  normalizeUser({
    ...fallbackUser,
    id: sessionUser?.id || fallbackUser.id,
    email: sessionUser?.email || fallbackUser.email,
    bio: fallbackUser?.profileCompletedAt ? fallbackUser.bio : '',
    avatarUrl: fallbackUser?.profileCompletedAt ? fallbackUser.avatarUrl : '',
    wallpaperUrl: fallbackUser?.profileCompletedAt
      ? fallbackUser.wallpaperUrl
      : '',
    name:
      ...
      fallbackUser.name,
    handle:
      sessionUser?.user_metadata?.handle ||
      fallbackUser.handle,
    birthDate:
      sessionUser?.user_metadata?.birth_date ||
      fallbackUser.birthDate ||
      '',
  });

export const sanitizeProfileHandle = (handle = '') =>
  `${handle}`
    .trim()
    .toLowerCase()
    .replace(/^@+/, '')
...
const cleanHandle = (handle = '') => sanitizeProfileHandle(normalizeHandle(handle));
...
export const mapUserToProfilePayload = (user = {}) => ({
  ...
  profile_completed_at: user.profileCompletedAt || null,
});

const getAuthRedirectUrl = () => {
  const explicitRedirect = process.env.EXPO_PUBLIC_SUPABASE_REDIRECT_URL?.trim();
  ...
  if (pathname.startsWith('/B-side-git/')) {
    return `${origin}/B-side-git/`;
  }
  ...
  try {
    ...
    return {
      ...
      message: formatSupabaseErrorMessage(
        error,
        'No pudimos recuperar la sesión actual.'
      ),
    };
  }
}
...
export async function registerWithEmail({ email, password, metadata = {} }) {
  ...
  try {
    ...
    return {
      ...
      message: formatSupabaseErrorMessage(
        error,
        'No pudimos registrar la cuenta.'
      ),
    };
  }
}
...
export async function sendMagicLink(email) {
  ...
  try {
    ...
    return {
      ...
      message: formatSupabaseErrorMessage(
        error,
        'No pudimos enviar el acceso por mail.'
      ),
    };
  }
}
...
export async function sendPasswordReset(email) {
  ...
  if (!client) {
    return {
      ok: false,
      message:
        'Supabase todavía no está configurado. Completá la conexión antes de recuperar la cuenta.',
    };
  }
  ...
  try {
    ...
    return {
      ...
      message: formatSupabaseErrorMessage(
        error,
        'No pudimos enviar el mail para restablecer la contraseña.'
      ),
    };
  }
}
...
export async function resendSignupVerificationEmail(email) {
  ...
  if (!client) {
    return {
      ok: false,
      message:
        'Supabase todavía no está configurado. Completá la conexión antes de reenviar el email.',
    };
  }
...
export async function signOutSession() {
  ...
  try {
    logSupabaseDebug('sign_out:start');
    ...
    logSupabaseDebug('sign_out:success');
    return { ok: true };
  } catch (error) {
    logSupabaseError('sign_out:error', error);
    return {
      ok: false,
      message: formatSupabaseErrorMessage(
        error,
...
export async function fetchProfileById(userId, fallbackUser = {}) {
  ...
  try {
    ...
    if (!data) {
      logSupabaseDebug('fetch_profile_by_id:missing', { userId });
      return {
        ok: true,
        profile: null,
        user: null,
    ...
    logSupabaseError('fetch_profile_by_id:error', error, { userId });
    return {
      ok: false,
...
export async function checkHandleAvailability(handle, currentUserId = '') {
  ...
  if (normalizedHandle.length < 3) {
    return {
      ok: false,
      available: false,
      message: 'Elegí un @ de al menos 3 caracteres.',
    };
  }

  const profileResult = await fetchProfileByHandle(normalizedHandle);
  ...
  if (
    ...
    profileResult.profile.id !== currentUserId
  ) {
    return {
      ok: true,
      available: false,
      message: `@${normalizedHandle} ya está en uso.`,
...
export async function createListeningEventRecord(userId, listeningEntry) {
  ...
  if (!userId || !payload.albumId || !payload.title) {
    return {
      ok: false,
      skipped: true,
      message: 'Faltan datos para sincronizar esta escucha.',
    };
  }
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
      logSupabaseDebug('listening_event:insert:start', {
        userId,
      ...
      logSupabaseDebug('listening_event:insert:ok', {
        userId,
        albumId: payload.albumId,
        backendId: data.id,
      });
      ...
      lastError = error;
      logSupabaseError('listening_event:insert:failed', error, {
        userId,
        albumId: payload.albumId,
        attempt,
      });
      ...
      if (attempt < 2) {
        await wait(LISTENING_EVENT_RETRY_DELAY_MS);
      }
    }
  }
  ...
  return {
    ...
    message: formatSupabaseErrorMessage(
      lastError,
      'Guardamos la escucha en este dispositivo, pero no pudimos sincronizar la racha.'
    ),
  };
}
...
export async function createBackendNotification({
  ...
  if (
    !NOTIFICATION_ALLOWED_TYPES.has(type) ||
    !NOTIFICATION_ALLOWED_ENTITY_TYPES.has(entityType)
  ) {
    return {
      ok: false,
      message: 'Tipo de notificación no permitido.',
    };
  }
  ...
  try {
    ...
    const resolvedActorId = actorId || sessionUserId;

    if (!sessionResult.ok || !accessToken || !sessionUserId) {
      return {
        ok: false,
        message: sessionResult.message || 'Necesitamos una sesión válida.',
      };
    }

    if (resolvedActorId !== sessionUserId) {
      return {
        ok: false,
        message: 'No podés crear notificaciones para otra cuenta.',
      };
    }

    logSupabaseDebug('notification:create:start', {
      recipientId,
    ...
    if (!responseData?.ok || !responseData?.notification) {
      throw new Error(
        responseData?.error || 'No pudimos crear la notificación.'
      );
    }
    ...
    logSupabaseDebug('notification:create:ok', {
      recipientId,
    ...
    return {
      ok: true,
      push: responseData.push || { delivered: 0, skipped: true },
...
export async function getAuthenticatedProfileSnapshot(fallbackUser = {}) {
  ...
  const profileResult = await fetchProfileById(
    ...
    mapSessionUserToFallback(sessionResult.session.user, fallbackUser)
  );

  if (!profileResult.ok) {
    logSupabaseError('auth_snapshot:profile_fetch_error', profileResult, {
    ...
    return {
      ...
      profile: null,
      user: mapSessionUserToFallback(sessionResult.session.user, fallbackUser),
      warning: profileResult.message,
    };
  }

  if (!profileResult.profile) {
    const fallbackProfileUser = {
      ...mapSessionUserToFallback(sessionResult.session.user, fallbackUser),
    ...
    logSupabaseDebug('auth_snapshot:repair_profile', {
      userId: sessionResult.session.user.id,
      handle: fallbackProfileUser.handle,
    });

    const repairResult = await upsertProfile(fallbackProfileUser);

    if (repairResult.ok) {
      return {
        ok: true,
        session: sessionResult.session,
        profile: mapUserToProfilePayload(repairResult.user),
        user: normalizeUser({
          ...repairResult.user,
          email:
            sessionResult.session.user.email || repairResult.user.email,
        }),
      };
    }

    logSupabaseError(
      'auth_snapshot:repair_profile_error',
      repairResult,
      { userId: sessionResult.session.user.id }
    );
    ...
    return {
      ...
      session: sessionResult.session,
      profile: null,
      user: normalizeUser(fallbackProfileUser),
      warning:
        repairResult.message ||
        'No pudimos reconstruir el perfil, pero la sesión sigue activa.',
    };
  }
  ...
  try {
    ...
    if (accessToken && refreshToken) {
      ...
      return {
        ...
        message: 'Tu acceso quedó confirmado. Ya podés seguir en B-Side.',
      };
    }
      ...
      return {
        ...
        message:
          verificationType === 'recovery'
            ? 'Validamos el enlace para recuperar la cuenta.'
            : 'Tu email quedó verificado. Ya podés seguir en B-Side.',
      };
    }
    ...
    return {
      ...
      message: formatSupabaseErrorMessage(
        error,
        'No pudimos completar el acceso desde el enlace.'
      ),
    };
  }
}
...
export async function requestAccountDeletion() {
  ...
  try {
    ...
    if (!accessToken || !userId) {
      return {
        ok: false,
        message: 'Necesitamos una sesión válida para borrar la cuenta.',
      };
    }

    logSupabaseDebug('delete_account:start', { userId });
    ...
    logSupabaseDebug('delete_account:success', { userId });
    return {
      ok: true,
      data,
      message:
        data?.message ||
        'Tu cuenta se marcó para borrado y se cerró la sesión en este dispositivo.',
    };
  } catch (error) {
    logSupabaseError('delete_account:error', error);
    return {
      ok: false,
